...
class DrivingClient(DrivingController):
    ...
    def __init__(self):
        ...
        self.full_road_width = 0
        self.interval = 0
        self.target_point_level = 7        
        # mark_obstacle은 직접 수정할것.
        
        self.max_val = 99999
        self.max_val_100 = 9999999
        self.bfs_level = 8 #1~10
    ...
    def make_decision(self, sensing_info, car_controls, curve_info) :
        #road racing
        min_route_data = self.calc_priority_route(sensing_info, curve_info)
        if self.debug_level >= self.debug_infos:
        ...
        for min_route in min_route_data:
            if min_route[self.bfs_level+2] < min_total_penalty: #priority penalty
                ...
                min_total_dist = min_route[self.bfs_level]
                min_total_angle = min_route[self.bfs_level+1]
                min_total_penalty = min_route[self.bfs_level+2]
        ...
        # brake
        brake_value = (math.pow(1.2, abs(car_controls.steering) * sensing_info.speed) -2)/100 # Rule of Thumb 1.2, 3, 5
        ...
        if min_total_angle * math.pow(sensing_info.speed,2)< 50000 * self.bfs_level:
            throttle_value *= 1
            if self.debug_level >= self.debug_controls_only:
                print("throttle_adj : * 1")

        elif min_total_angle * math.pow(sensing_info.speed,2) < 100000 * self.bfs_level:
            throttle_value *= 0.7
            if self.debug_level >= self.debug_controls_only:
    ...
    def calc_road_position(self, sensing_info, curve_datas):
        ...
        if len(sensing_info.track_forward_obstacles) != 0:
            for obs_info in sensing_info.track_forward_obstacles:
                ...
                curve_matrix_index = min(max(int(obs_dist / 10), 0)+1, 9)
    ...
    def calc_priority(self, calced_dist, calced_angle):
        ...
        return calced_dist * math.pow(1.5, abs(calced_angle)/200)

    def calc_priority_route(self, sensing_info, curve_info):
        ...
        result = []
        for index in range(self.target_point_level):
            result.append(self.calc_priority_bfs(curve_info, index, curve_info[0][0][index][0], curve_info[0][0][index][1], curve_info[0][0][index][2], [index]))

            # first_target_route = self.calc_priority_bfs(curve_info, 0, curve_info[0][0][0][2], [0])
            # second_target_route = self.calc_priority_bfs(curve_info, 1, curve_info[0][0][1][2], [1])
            # third_target_route = self.calc_priority_bfs(curve_info, 2, curve_info[0][0][2][2], [2])
            # fourth_target_route = self.calc_priority_bfs(curve_info, 3, curve_info[0][0][3][2], [3])
            # fifth_target_route = self.calc_priority_bfs(curve_info, 4, curve_info[0][0][4][2], [4])

        #result = [first_target_route, second_target_route, third_target_route, fourth_target_route, fifth_target_route]
        return result

    def calc_priority_bfs(self, curve_info, src_index, distance, angle, penalty, chosen_sequence):        
