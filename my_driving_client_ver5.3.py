...
class DrivingClient(DrivingController):
    ...
    def make_decision(self, sensing_info, car_controls, curve_info) :
        ...
        throttle_value =1 ##임시로직

        if min_total_angle < 100:
            throttle_value*=1
        elif min_total_angle <150:
            throttle_value*=0.97
        else:
            throttle_value*=0.95
        ...
        # brake
        brake_value = (math.pow(1.2, abs(car_controls.steering) * sensing_info.speed) -10)/100 # Rule of Thumb 1.2, 3, 5
    ...
    def mark_obstacles(self, obs_dist, obs_to_middle, interval, curve_datas, curve_matrix_index_adj, curve_matrix_index):
        for index in range(curve_matrix_index_adj, curve_matrix_index+1):
            ...
            obs_left = obs_to_middle - 1.25
            obs_right = obs_to_middle + 1.25
    ...
    def calc_priority(self, calced_dist, calced_angle, area_index, target_index):
        ...
        result *= math.pow(1.2,(self.ROUTE_CALC_LEVEL- area_index))
        result *= (1 + abs(target_index - self.target_point_level/2)*0.2)
        result *= 0.000001
    ...
    def stabilize(self, car_controls, min_info):
        ...
        if abs(sum_steering) >= 20*self.HIST_LEVEL*0.015 and car_controls.steering*sum_steering <0: ##0.01 = steer 조정값
