...
class DrivingClient(DrivingController):
    ...
    def __init__(self):
        ...
        self.ROUTE_CALC_LEVEL = 4 #1~10 전방 구간 예측 레벨. 늘릴경우 너울치는 현상이 심해지고, 도로 바깥의 지름길로 주행하려는 성질이 강해짐.
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        curve_info = [] # 전방 구간들의 커브정보. [구간][구간의 시작점][구간의 종료점][dist, angle, priority]
        curve_info = self.calc_target_road(sensing_info) # 
    ...
    def make_decision(self, sensing_info, car_controls, curve_info) :
        #road racing
        min_route_data = self.calc_routes(sensing_info, curve_info) #route_calc_level까지 구한 최적구간. 
        if self.DEBUG_LEVEL >= self.DEBUG_INFOS:
            print("### min_route_data calculated ###")

        #min_route_data 예시
        #route_calc_level이 3일때
        #[0, 2, 2, distance 합, angle 합, penalty 합]
        #[1, 2, 2, distance 합, angle 합, penalty 합]
        #[2, 3, 4, distance 합, angle 합, penalty 합]
        #[0번구간의 target_point, 1번구간의 target_point, 2번 구간의 target_point, distance 합, angle 합, penalty 합]
        #..

        min_total_dist = self.MAX_VAL_100
        min_total_angle = self.MAX_VAL_100
        min_total_penalty = self.MAX_VAL_100
        min_info = [0, self.MAX_VAL, 0, self.MAX_VAL] #index, dist, angle, penalty
        loop_index = 0    
        ...
        for min_route in min_route_data:
            if min_route[self.ROUTE_CALC_LEVEL+2] < min_total_penalty: #penalty
                min_info[1] = curve_info[0][0][loop_index][0] #선택된 min route의 0번구간 target_point까지 dist
                min_info[2] = curve_info[0][0][loop_index][1] #선택된 min route의 0번구간 target_point까지 angle
                min_info[3] = curve_info[0][0][loop_index][2] #선택된 min route의 0번구간 target_point까지 penalty
        ...
        if self.DEBUG_LEVEL >= self.DEBUG_CONTROLS_ONLY:
            print("min_route! target = {}, dist = {}, angle = {}, penalty = {}".format(round(min_info[0],2), round(min_info[1],2), round(min_info[2],2), round(min_info[3],2)))

        # 전방 커브값의 합이 일정 이상일경우 throttle 조정을 위한 값.
        for angle in sensing_info.track_forward_angles:
            min_total_angle += abs(angle)

        #steer        

        steer_value = min_info[2] * 0.015  # 0번구간 target_point 까지의 angle * steer값
        ...
        if min_total_angle < 90:
            throttle_value*=1
        elif min_total_angle <140:
            throttle_value*=0.94
        else:
            throttle_value*=0.86
        ...
        brake_value = (math.pow(1.2, abs(car_controls.steering) * sensing_info.speed) -10)/100 
        ...
        self.stabilize(car_controls, min_info)    
